...
public class matrixBlob {
   ...
   public matrixBlob(final blob b) {
      pixelCount = b.size;
   ...
   public matrixBlob scaleMatrixBlob(final blob b, final int height) {
      try {

         final int[][] orig = b.matrixVersion.matrix;
         final double mult = (double) height / orig[0].length;
         final int[][] newOne = new int[(int) (mult * orig.length)][height];
         int newPixelCount = 0;
         for (int i = 0; i < newOne.length; i++) {
            for (int j = 0; j < newOne[0].length; j++) {
               try {
                  ...
                  // System.out.print("mult"+mult+" f:"+f+" p:"+p+" newOne:"+newOne.length+"
                  // "+newOne[0].length+"b: "+orig.length+" "+orig[0].length+"\n");
                  if (f > orig.length - 1) {
                     f = orig.length - 1;
                  }
   ...
   public matrixBlob(final int[][] mat, final int pixelCount) {
      this.matrix = mat;
      this.pixelCount = pixelCount;
   }

   public double compareWithImplicitScaling(final matrixBlob b) {
      try {
         final double toret = 0.0;
         final int lcm = matrixBlob.determineLCM(b.matrix[0].length, this.matrix[0].length);

         final int aMultiplier = lcm / this.matrix[0].length;
         final int bMultiplier = lcm / b.matrix[0].length;
         ...
         // System.out.print("matches:"+ matches+" lcm:"+lcm+"
         // b:"+b.matrix.length+"x"+b.matrix[0].length+":"+bMultiplier+ "
         // a:"+this.matrix.length+"x"+matrix[0].length+":"+aMultiplier+"\n");
         return (double) matches / total;
         /*
          * if(pixelCount>b.pixelCount) return
          * (double)matches/(lcm*matrix.length*aMultiplier); else return
          * (double)matches/(lcm*b.matrix.length*bMultiplier);
          */
      } catch (final Exception e) {
         e.printStackTrace();
         return 0.0;
   ...
   public double compareWithScaling(final matrixBlob b) {
      ...
      try {
         double toret = 0.0;
         final matrixBlob[] res = matrixBlob.scaleUsingCommonFactor(this, b);
         if (res.length != 2) {
            return 0.0;
         }
         final matrixBlob A = res[0];
         final matrixBlob B = res[1];
         // A.writeGlyph("b"+System.currentTimeMillis(), B);
         toret = A.compareTo(B);
         /*
          * if(toret>.6){ A.writeGlyph("b"+System.currentTimeMillis(), B);
   ...
   public double compareTo(final matrixBlob b) {
      try {
         int matches = 0;
   ...
   public void drawBlob(final BufferedImage img, final int x, final int y, final int color) {
   ...
   public double compareToWithAdjust(final matrixBlob b) {
      ...
      for (int i = 0; i < ySize; i++) {
         for (int j = 0; j < xSize; j++) {
            if (matrix[i][j] == 1) {
               count(i, j, pixels);
               if (pixels.size() > 25) {
                  final blob tmp = new blob();
                  final blob[] newOne = new blob[toret.length + 1];
   ...
   public blob[] breakBlob(final int width, final int height) {
      int bestBet = 0;
      ...
      // if the thinnes point was 1 pixel thick, white that pixel out and rerun the
      // blob pixel accumulator, should get 2 blobs.
      if (bestBet > 0) {
         for (int j = 0; j < this.ySize; j++) {
            matrix[j][bestBet] = 0;
         }
         final blob[] toadd = findblobs();
         return toadd;
   ...
   public void count(final int currentx, final int currenty, final Vector<pixel> pixels) {
   ...
   public BufferedImage writeGlyph(final String prefix) throws IOException {
   ...
   public BufferedImage writeGlyph(final String prefix, final matrixBlob mb2) throws IOException {
      BufferedImage toret = ImageHelpers.readAsBufferedImage("/usr/blankimg.jpg");
      toret = ImageHelpers.scale(toret, this.matrix.length + mb2.matrix.length,
            this.matrix[0].length + mb2.matrix[0].length);
      ...
      for (int i = 0; i < mb2.matrix.length; i++) {
         for (int j = 0; j < mb2.matrix[0].length; j++) {
            if (mb2.matrix[i][j] == 1 && toret.getWidth() > i + matrix.length
                  && toret.getHeight() > j + matrix[0].length) {

               toret.setRGB(j + matrix[0].length, i + matrix.length, 0x0000ff);
            }
         }
      }
      // toret=toret.getSubimage(0, 0, matrix.length+mb2.matrix.length,
      // matrix[0].length+mb2.matrix[0].length);
      ImageIO.write(toret, "jpg",
            new File("/usr/glyphs/" + prefix + System.currentTimeMillis() + Thread.currentThread().getId() + ".jpg"));
      return toret;
   }

   /**
    * Returns an array containing 2 elements, matrix a at index 0 and matrix b at
    * index 1
    */
   public static matrixBlob[] scaleUsingCommonFactor(final matrixBlob a, final matrixBlob b) {

      final matrixBlob[] toret = new matrixBlob[2];

      final int commonFactor = determineLCM(a.matrix.length, b.matrix.length); // lazy wasteful way to calculate this,
                                                                               // but fine for testing purposes.
      // System.out.print(a.matrix.length+":"+b.matrix.length+":"+commonFactor+"\n");
      final int aMultiplier = commonFactor / a.matrix.length;
      final int bMultiplier = commonFactor / b.matrix.length;
      final int[][] matrixA = new int[commonFactor][aMultiplier * a.matrix[0].length];

      final int[][] matrixB = new int[commonFactor][bMultiplier * b.matrix[0].length];
      int pixelsA = 0;
      int pixelsB = 0;
