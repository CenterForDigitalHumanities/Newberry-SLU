...
public class imageProcessor {
    ...
    * @param h
    */
   public imageProcessor(final BufferedImage img, final int h) {
      untouchedImage = img;
      orig = img;
      maxHeight = h;
   }

   public static void setThesholdMethod(final int meth) {
    ...
    *
    * @param doBlobExtract Use an additional preprocessing method that looks for
    *                      likely characters, and only uses them in the column/line
    *                      detection to the exclusion of all other content in the
    *                      image. Best for microfilm images.
    * @return Vector containing all lines, null in case of unrecoverable error
    */
   public List<line> detectLines(final boolean doBlobExtract) {
      ...
      final BufferedImage bin = ImageHelpers.scale(ImageHelpers.binaryThreshold(untouchedImage, 4), maxHeight);
      ...
      if (doBlobExtract) {
         ...
         for (int i = 0; i < bin.getWidth(); i++) {
            for (int j = 0; j < bin.getHeight(); j++) {
               if (bin.getRGB(i, j) != -1) {
                  final blob thisOne = new blob(i, j);
                  thisOne.copy = orig;
                  if (blobs.size() % 3 == 0) {
                  ...
                  if (thisOne.size > 5) {
                     blobs.add(thisOne);
                     if ((thisOne.getSize() - (thisOne.getSize() % 10)) != 5000
                           && (thisOne.getSize() - (thisOne.getSize() % 10)) != 0) {
      ...
      for (int i = 0; i < lines.size(); i++) {
         final line col = lines.get(i);
         final BufferedImage storedBin = ImageHelpers.binaryThreshold(stored, 4);
         final BufferedImage colOnly = storedBin.getSubimage(col.getStartHorizontal(), col.getStartVertical(),
               col.getWidth(), col.getDistance());
         final Detector d = new Detector(colOnly, colOnly);
         // d.debugLabel = imageFile.getName();
         d.detect(true);
         LOG.log(Level.INFO, "total lines in col is {0}", d.lines.size());
         for (int j = 0; j < d.lines.size(); j++) {
            final line r = d.lines.get(j);
            allLines.add(r);
            ...
            if (j % 2 == 1) {
               final int color = 0x0000ff;
               stored = highlightBlock(stored, r.getStartHorizontal(), r.getStartVertical() - r.getDistance(),
                     r.getDistance(), r.getWidth(), color);
            } else {
    ...
    * @return
    */
   private static BufferedImage createViewableVerticalProfile(final BufferedImage bin, final String fileName,
         final List<line> v) {
      final int white = 0xff000000;
      final int black = 0xffffffff;
      final BufferedImage toret = cloneImage(bin);
      // writeDebugImage(bin, "createViewableVerticalProfile.bin");
      final int[] vals = new int[toret.getWidth()];
      ...
      final int[] ordered = Arrays.copyOf(vals, vals.length);
      Arrays.sort(ordered);
      int x = 0;
      ...
      for (int i = 0; i < vals.length; i++) {
         if (vals[i] <= median) {
            ...
            if (x > 0) {
               ...
               l.setWidth(i - x);
               LOG.log(Level.INFO, "I: New line {5}/{0}: {1},{2},{3},{4}", new Object[] { i, l.getStartHorizontal(),
                     l.getStartVertical(), l.getWidth(), l.getDistance(), lines.size() });
               lines.add(l);
               x = 0;
            }
         } else {
            ...
            if (i == vals.length - 1) {
               ...
               l.setWidth(i - x);
               lines.add(l);
               LOG.log(Level.INFO, "II: New line {5}/{0}: {1},{2},{3},{4}", new Object[] { i, l.getStartHorizontal(),
                     l.getStartVertical(), l.getWidth(), l.getDistance(), lines.size() });
               x = 0;
            }
         }
      ...
      final int minColWidth = bin.getWidth() / MINIMAL_COLUMN_FRACTION;
      for (int j = 0; j < lines.size(); j++) {
         final line l = lines.get(j);
         // merging needs to consider the magnitude of the difference from median
         if (j > 0) {
            final line prev = lines.get(j - 1);
            if (l.getStartHorizontal() - prev.getStartHorizontal() - prev.getWidth() < MINIMAL_GAP
                  && (l.getWidth() < minColWidth || prev.getWidth() < minColWidth
                        || l.getStartHorizontal() - prev.getStartHorizontal() - prev.getWidth() < 3)) {
               prev.setWidth(l.getStartHorizontal() + l.getWidth() - prev.getStartHorizontal());
               lines.remove(l);
               j--;
            } else {
               // System.out.print(" gap is "+l.getStartHorizontal()+" "+
               // prev.getStartHorizontal()+" "+prev.getWidth()+"\n");
            }
         }
      ...
      for (int j = 0; j < lines.size(); j++) {
         ...
         if (l.getWidth() < minColWidth) {
            /*
             * if(l.getWidth()!=0) { finalizedLines.add(l); l.setStartVertical(0);
             * l.setDistance(bin.getHeight()/2);
             * imageHelpers.highlightBlock(bin,l.getStartHorizontal(), l.getStartVertical(),
             * l.getDistance(), l.getWidth(), 0x0000ff); }
             */
         } else {
            v.add(l);
            ...
            // writeDebugImage(highlightBlock(bin, l.getStartHorizontal(),
            // l.getStartVertical(), l.getDistance(), l.getWidth(), 0xff0000),
            // "createViewableVerticalProfile.column" + (v.size() - 1));
         }
      }
    ...
    * @return
    */
   private static BufferedImage highlightBlock(final BufferedImage img, final int x, final int y, final int height,
