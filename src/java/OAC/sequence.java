...
public class sequence {
    ...
    public String getSequenceElement(final int position) {
     ...
     */
    public sequence(final URL[] sequenceUrl, final String format) throws IOException {
        final Model sequenceModel = ModelFactory.createDefaultModel();

        // use the FileManager to find the input file
        final InputStreamReader in = null;
        int positionCounter = 1;
        for (final URL sequenceUrl1 : sequenceUrl) {
...
//    in= new InputStreamReader(sequenceUrl[i].openStream());
            ...
            final Property seqProperty = sequenceModel.getProperty("http://www.openarchives.org/ore/terms/aggregates");
            final ResIterator sequences = sequenceModel.listResourcesWithProperty(seqProperty);//.listSubjects();
            ...
            if (m3UrlString.compareTo("") != 0) {
                final URL m3Url = new URL(m3UrlString + ".xml");
                ...
                while (results.hasNext()) {
                    ...
                    final Resource r = qs.get("predicate").asResource();
                    //System.out.print(" stats:"+r.getId()+" :"+r.isAnon()+", "+r.isLiteral()+", "+r.isResource()+", "+r.isURIResource()+"\n");

                    //System.out.print(qs.get("predicate")+"\n");
                    final Query innerQuery = QueryFactory.create("select  ?pred where { <" + r.getURI() + "> " + "<http://purl.org/dc/elements/1.1/title> ?pred}");
                    qe = QueryExecutionFactory.create(innerQuery, m3Model);
                    final ResultSet innerResults = qe.execSelect();
                    while (innerResults.hasNext()) {
                        final QuerySolution qs2 = innerResults.next();
                        System.out.print("Title:" + qs2.get("pred") + "\n");

                        final Query innerQuery3 = QueryFactory.create("select ?pred  where { ?pred <http://www.openannotation.org/ns/hasTarget> <" + r.getURI() + "> }");
                        ...
                        while (innerResults2.hasNext()) {
                            ...
                            //now get the image
                            final Query innerQuery4 = QueryFactory.create("select ?pred  where { <" + innerqs2.getResource("pred").getURI() + "> <http://www.openannotation.org/ns/hasBody> ?pred }");
                            qe = QueryExecutionFactory.create(innerQuery4, sequenceModel);
                            final ResultSet innerResults4 = qe.execSelect();
                            if (innerResults4.hasNext()) {
                                final QuerySolution innerqs4 = innerResults4.next();
            ...
            for (; !listItem.equals(RDF.nil);) {
                ...
                final Resource entry = listItem.getProperty(RDF.first).getResource();
                System.out.print(entry.getURI() + "\n");

                final view v = new view(sequenceModel, entry.getURI());
//System.out.print(v.getImages().length+"\n");
    ...
    public static void main(final String[] args) {
        try {
            final String w = "http://67.23.4.192/NormalSequence.xml";
