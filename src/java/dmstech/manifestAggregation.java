...
public class manifestAggregation {
     ...
     * @throws IOException 
     */
    public static String [] getManifestUrls(final String url) throws MalformedURLException, IOException {
        String[] toret = null;
        final Stack<String> urls = new Stack();
        String format = "";
        final URL m3Url = new URL(url);
        ...
        while (results.hasNext()) {
            ...
            queryString = "prefix ore:<http://www.openarchives.org/ore/terms/> select * where{ ?sub ore:describes <"
                    + qs.get("obj") + "> }";
            query = QueryFactory.create(queryString);
            qe = QueryExecutionFactory.create(query, m3Model);
            final ResultSet results2 = qe.execSelect();
